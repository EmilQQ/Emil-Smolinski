...
private fun ScreenPreview() {
    FilmotekaTheme {
        FilmEditScreen(film = FilmRepository.films[1], onSave = { }, onNavigationUp = { })
    }
}
...
fun FilmEditScreen(
    ...
    var releaseDate by remember { mutableStateOf(film.releaseDate.format(dateFormatter)) }
//    var category by remember { mutableStateOf(film.category.name) }
    var status by remember { mutableStateOf(film.status.name) }
    var selectedImageId by remember { mutableStateOf<Uri?>(null) }
    var expandedCategory by remember { mutableStateOf(false) }
    var pickedCategory by remember { mutableStateOf(film.category) }
    ...
    Scaffold(
                ...
                onClick = {
                    if (title.isBlank()) {
                        ...
                        val editedFilm = film.copy(
                            ...
                            posterId = selectedImageId ?: film.posterId
                        )
                        onSave(editedFilm)
                    }
                },
        ...
        Column(
            ...
            Image(
                ...
                } ?: rememberAsyncImagePainter(model = film.posterId),
